/*
 * https://techiedelight.com/practice/?problem=SubsetSum
 *
 * */
#include <bits/stdc++.h>
...
bool solve(int i, const int k, const vector<int> &nums, int sum) {
    if (i == nums.size()) {
        return sum == k;

    }
    return solve(i + 1, k, nums, sum + nums[i]) || solve(i + 1, k, nums, sum);
}
...
int solve(ll num) {
    ...
    return memo[num] = solve(num - 2020) || solve(num - 2021);
}
...
int main() {
    ...
    while (t--) {
        ...
        auto minE = min_element(v.begin(), v.end());
        int lmax = maxE - v.begin();
        int lmin = minE - v.begin();
        int rmax = v.end() - maxE - 1;
        int rmin = v.end() - minE - 1;
//        cout << lmax << " " << rmax << endl;
//        cout << lmin << " " << rmin << endl;
        cout << min(lmax + rmin + 2, min(rmax + lmin + 2, min(max(rmax, rmin), max(lmax, lmin)) + 1)) << endl;
    }
    return 0;
}