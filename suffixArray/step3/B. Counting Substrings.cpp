...
int main() {
    ...
    while ((1 << k) < n) {
        ...
        for (int i = 1; i < n; ++i) {
            ...
            pair<int, int> enew = {c[p[i]], c[(p[i] + (1 << k)) % n]};
            if (prev == enew) {
    ...
    while (t--) {
        ...
        cin >> q;
        int l = 0, r = n - 1;
        int equalM = -1, equalLeft = -1, equalRight = -1;
        {
            while (l <= r) {
                int m = (r + l) / 2;
                string subS = s.substr(p[m], min(n - p[m], (int) q.size()));
                int cmp = subS.compare(q);
                if (cmp == 0) {
                    equalM = m;
                    break;
                } else if (cmp < 0) {
        ...
        equalRight = l;
        cout << equalRight - equalLeft + 1 << endl;
