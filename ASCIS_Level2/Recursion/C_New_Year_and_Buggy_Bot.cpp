...
int main() {
    ...
    vector<char> directions = {'0', '1', '2', '3'};
    ll cnt = 0;
    do {
        cur = start;
        for (char c: s) {
            if (c == directions[0]) {
                if (isValid(cur.first + 1, cur.second,n,m,v))
                    cur.first++;
                else
                    break;
            } else if (c == directions[1]) {
                if (isValid(cur.first - 1, cur.second,n,m,v))
                    cur.first--;
                else
                    break;
            } else if (c == directions[2]) {
                if (isValid(cur.first, cur.second - 1,n,m,v))
                    cur.second--;
                else
                    break;
            } else {
