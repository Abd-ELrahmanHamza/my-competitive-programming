...
bool bars(vi &v, int index, int sum, int nn) {
    if (sum == nn)return true;
    if (index >= v.size() || sum > nn)return false;

    bool path1 = bars(v, index + 1, sum + v[index], nn);
    bool path2 = bars(v, index + 1, sum, nn);
//    cout << 1 << endl;
    return path1 || path2;
}
