...
int main() {
    ...
    for (int i = 0; i < 2 * n; ++i) {
        cin >> x;
        if (v[x].first == -1)v[x].first = i + 1;
        else v[x].second = i + 1;
    }

    int sasha = 1, dima = 1;ll cnt = 0;
    for (int i = 1; i <= n; ++i) {
        if (abs(v[i].first - sasha) + abs(v[i].second - dima) >= abs(v[i].second - sasha) + abs(v[i].first - dima)) {
            cnt += abs(v[i].second - sasha) + abs(v[i].first - dima);
            sasha = v[i].second;
            dima = v[i].first;
        } else {
            cnt += abs(v[i].first - sasha) + abs(v[i].second - dima);
            sasha = v[i].first;
            dima = v[i].second;
        }
    }
