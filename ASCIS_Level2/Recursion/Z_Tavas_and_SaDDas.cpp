...
void tavas_and_saddas(ll num, int n, vi &v) {
    if (num > n)return;
    v.push_back(num);
    tavas_and_saddas(num * 10 + 4, n, v);
    tavas_and_saddas(num * 10 + 7, n, v);
}
...
int main() {
    ...
    cin >> n;
    vi v;
    tavas_and_saddas(0, n, v);
    sort(v.begin(), v.end());
    cout << std::find(v.begin(), v.end(), n) - v.begin();
    return 0;
}