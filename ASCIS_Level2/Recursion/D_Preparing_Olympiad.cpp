...
bool valid(vector<int> &selected, vector<int> &all) {
    ...
    return sum >= l && sum <= r && maxx - minn >= x;
}

void solve(vector<int> selected, vector<int> all, int index) {
    if (!selected.empty() && valid(selected, all) && formed.count(selected) == 0) {
        cnt++;
        formed.insert(selected);
    }
    if (index == all.size())return;
    solve(selected, all, index + 1);
