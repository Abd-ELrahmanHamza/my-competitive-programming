...
ll number_of_divisors_mod(ll n, ll mod) {
    set<ll> dummySet;
    unordered_map<ll, ll> uMap = prime_factors_map(n, dummySet);
    ll result = 1;
    for (auto p: uMap) result = ((result % mod) * ((p.second + 1) % mod)) % mod;
    return result;
}
...
int main() {
    ...
    string s1, s2;
    cin >> s1 >> s2;
    int c1p = 0, c1n = 0, c2p = 0, c2n = 0, cq = 0;
    ...
    int neededp = c1p - c2p;
    int neededn = c1n - c2n;
    if (neededn < 0 || neededp < 0) {
        cout << fixed << setprecision(10) << 0.0;
        return 0;
    }
    cout << fixed << setprecision(10) << double(nCr(cq, neededn)) / pow(2, cq);
    return 0;
}