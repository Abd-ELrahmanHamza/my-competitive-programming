...
void dfs(int i, int j, int &cnt, bool &invalid) {
    visited[i][j] = true;
    if (i == 0 || i == n - 1 || j == 0 || j == m - 1)invalid = false;
    cnt++;
    for (int l = 0; l < 4; ++l) {
        int x = i + directionx[l];
        int y = j + directiony[l];
        if (valid(x, y) && !visited[x][y] && adjList[x][y] != '*')
            dfs(x, y, cnt, invalid);
    }
}

void dfsRemoveWater(int i, int j) {
    adjList[i][j] = '*';
    for (int l = 0; l < 4; ++l) {
        int x = i + directionx[l];
        int y = j + directiony[l];
        if (valid(x, y) && adjList[x][y] != '*')
            dfsRemoveWater(x, y);
    }
}
...
int connectedComponents() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (!visited[i][j] && adjList[i][j] == '.') {
                int cnt = 0;
                bool invalid = true;
                dfs(i, j, cnt, invalid);
                if (invalid)result.push_back(make_pair(cnt, make_pair(i, j)));
            }
        }
    }
    ...
    int toRemove = result.size() - k;
    int r = 0;
    for (int i = 0; i < toRemove; ++i) {
        r += result[i].first;
        dfsRemoveWater(result[i].second.first, result[i].second.second);
    }
    return r;
}
...
int main() {
    ...
    cout.tie(nullptr);
    cin >> n >> m >> k;
    adjList = vector<vector<char>>(n, vector<char>(m));
