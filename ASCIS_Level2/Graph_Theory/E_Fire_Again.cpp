...
//    visited[i][j] = true;
//    forest[i][j] = min(forest[i][j], cnt);
//    for (int l = 0; l < 3; ++l) {
//        for (int i1 = 0; i1 < 3; ++i1) {
//            if (valid(i + direction[l], j + direction[i1]) && !visited[i + direction[l]][j + direction[i1]])
//                dfs(i + direction[l], j + direction[i1], cnt + 1);
//        }
//    }
//}
...
int main() {
    ...
    cin >> n >> m >> k;
    burned = vector<pair<int, int>>(k);
    forest = vector<vector<int>>(n, vector<int>(m, INT_MAX));
    ...
    pair<int, int> lastBurned;
    int lastBurnedCnt = INT_MIN;

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (lastBurnedCnt < forest[i][j]) {
                lastBurnedCnt = forest[i][j];
                lastBurned.first = i + 1;
                lastBurned.second = j + 1;
            }
//            cout << forest[i][j] << " ";
        }
//        cout << endl;
    }
    cout << lastBurned.first << " " << lastBurned.second;
    return 0;
}