...
vi directionx = {-1, 1, 0, 0};
vi directiony = {0, 0, -1, 1};
int n, m;
int destx, desty;
bool validAns = false;
...
void testAns(vector<vector<char>> &graph, int i, int j) {
    if (graph[i][j] == 'X') {
        validAns = true;
        return;
    }
    for (int k = 0; k < 4; ++k) {
        int x = directionx[k];
        int y = directiony[k];
        if (!(x == 0 && y == 0) && valid(i + x, j + y) && graph[i + x][j + y] == '.') {
            validAns = true;
        }
    }
...
void dfsMatrix(vector<vector<char>> &graph, int i, int j) {

    if (i == destx && j == desty && graph[i][j] == 'X') {
        validAns = true;
    }

    if (graph[i][j] == 'X' || validAns)return;
    graph[i][j] = 'X';
    for (int k = 0; k < 4; ++k) {
        if (valid(i + directionx[k], j + directiony[k]))
            dfsMatrix(graph, i + directionx[k], j + directiony[k]);
    }
}
...
int main() {
    ...
    int ii, ij;
    cin >> ii >> ij >> destx >> desty;
    ii--;
    ij--;
    destx--;
    desty--;
    graph[ii][ij] = '.';
    dfsMatrix(graph, ii, ij);
    if (validAns)cout << "YES";
    else cout << "NO";
    return 0;
}