...
int main() {
    ...
    for (ll x: f) {
        ...
        auto it3 = it2 - 1;
        ll diff1 = (it != b.end() ? abs(x - *it) : LONG_LONG_MAX);
        ll diff2 = (it2 != b.end() ? abs(x - *it2) : LONG_LONG_MAX);
        ll diff3 = (it3 != b.begin() - 1 ? abs(x - *it3) : LONG_LONG_MAX);
        ll minE = min(diff1, min(diff2, diff3));
        ll result = -1;
        if ((it3 != b.begin() - 1) && abs(x - *it3) == minE)
            result = *it3;
        if ((it != b.end()) && abs(x - *it) == minE)
            result = *it;
        if ((it2 != b.end()) && abs(x - *it2) == minE)
            result = *it2;
        cout << result << endl;
    }
    return 0;
