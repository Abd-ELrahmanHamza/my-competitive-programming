...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        for (int i = 0; i < s.size(); ++i) {
            int num = s[i] - '0';
            if (turn) {
                if ((int) ceil(num / 2.0) != 0 || num1.size() != 0)
                    num1 = num1 + char(ceil(num / 2.0) + '0');
                if (num / 2 != 0 || num2.size() != 0)
                    num2 = num2 + char((num / 2) + '0');
            } else {
                if ((int) ceil(num / 2.0) != 0 || num2.size() != 0)
                    num2 = num2 + char(ceil(num / 2.0) + '0');
                if (num / 2 != 0 || num1.size() != 0)
                    num1 = num1 + char((num / 2) + '0');
            }
            if (num & 1)turn = !turn;
        }
