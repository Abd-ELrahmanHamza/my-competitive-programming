...
bool sortHelp(const pair<int, int> &p1, const pair<int, int> &p2) {
...
int main() {
    ...
    while (t--) {
        ...
        while (n--) {
            ...
            alarms.push_back(make_pair(resH, resM));
        }
        sort(alarms.begin(), alarms.end(), sortHelp);
//        for (int i = 0; i < alarms.size(); ++i) {
//            cout << alarms[i].first << " " << alarms[i].second << endl;
//        }
//        cout << endl;
        cout << alarms.back().first << " " << alarms.back().second << endl;
    }
    return 0;
}