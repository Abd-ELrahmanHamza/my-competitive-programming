...
struct Compare {
    bool operator()(pair<ll, int> const &p1, pair<ll, int> const &p2) {
...
bool comp(pair<ll, int> const &p1, pair<ll, int> const &p2) {
...
//    }
    ...
    for (int i = 1; i < n; ++i) {
        ...
        indices.insert(v[i].second);
    }
    for (int x: indices) {
        s << x + 1;
        s << ' ';
    }
    for (int j = 0; j < v[n - 1].first; ++j) {
        cout << s.str();
    }
    return 0;
}