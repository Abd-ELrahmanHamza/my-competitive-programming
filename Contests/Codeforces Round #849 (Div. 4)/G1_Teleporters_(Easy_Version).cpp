...
bool good(ll greatest, ll secGreatest, ll h, ll m) {
    if (m & 1)return (m / 2) * greatest + (m / 2) * secGreatest + greatest >= h;
    return (m / 2) * greatest + (m / 2) * secGreatest >= h;
}
...
int LIS(vi &v, int i) {
    ...
    for (int j = i + 1; j < v.size(); ++j) {
        if (v[j] > v[i])
            maxSol = max(maxSol, 1 + LIS(v, j));
    }
    return memo[i] = maxSol;
}
...
//        printV(v);
//        cout << endl;
        cout << upper_bound(v.begin(), v.end(), c) - v.begin() << endl;
    }
    return 0;
}