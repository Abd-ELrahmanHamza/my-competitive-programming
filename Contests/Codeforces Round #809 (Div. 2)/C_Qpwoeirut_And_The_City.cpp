...
int extractNum(int start, string s, int &end) {
    int result = 0;
    int i = start;
...
pair<ll, ll> minPair(const pair<ll, ll> &p1, const pair<ll, ll> &p2) {
    if (p1.second > p2.second)return p1;
    else if (p2.second > p1.second)return p2;
    else return min(p1, p2);
}

pair<ll, ll> solve(ll cost, ll cnt, int index) {
    if (index >= n - 1) {
        return make_pair(cost, 0);
    }
    if (mem[index].first != INT_MAX) {
        return mem[index];
    }

    int curCost = max(v[index - 1] - v[index], v[index + 1] - v[index]);
    if (curCost < 0)curCost = 0;
    else curCost++;
//    cout << "Cur cost : " << curCost << endl;
    pair<ll, ll> p1 = solve(cost, cnt + 1, index + 2);
    pair<ll, ll> p2 = solve(cost, cnt, index + 1);
    p1.second += 1;
    p1.first += curCost;
//    cout << "p1 = " << p1.first << " " << p1.second << endl;
//    cout << "p2 = " << p2.first << " " << p2.second << endl;
    if (p1.second > p2.second)return mem[index] = p1;
    else if (p2.second > p1.second)return mem[index] = p2;
    else return mem[index] = min(p1, p2);
    return mem[index] = minPair(p1, p2);
}
...
int main() {
    ...
    while (t--) {
        cin >> n;
        v = vi(n);
        mem = vector<pair<ll, ll>>(n, {INT_MAX, INT_MIN});
        readV(v);
        auto p = solve(0, 0, 1);
        cout << p.first << endl;
    }
