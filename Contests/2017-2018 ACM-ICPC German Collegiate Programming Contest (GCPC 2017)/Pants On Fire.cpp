...
//    freopen("gcd.in", "r", stdin);
    ...
    for (int i = 0; i < n; ++i) {
        string c1, are, worse, than, c2;
        cin >> c1 >> are >> worse >> than >> c2;
        adj[c1].push_back(c2);
    }
    for (int i = 0; i < m; ++i) {
        unordered_map<string, bool> visited;
        ...
        string c1, are, worse, than, c2;
        cin >> c1 >> are >> worse >> than >> c2;
        bool res1 = pantsDfs(c1, c2, adj, visited);
        if (res1) {
            cout << "Fact\n";
            continue;
        }
        visited.clear();
        bool res2 = pantsDfs(c2, c1, adj, visited);
        if (res2)cout << "Alternative Fact\n";
