...
int main() {
    ...
    while (t--) {
        ...
        string str;
        cin >> n >> str;
        vector<pair<char, int>> s, r;
        int size = 1;
        for (int i = 1; i < n; ++i) {
            if (str[i] != str[i - 1]) {
                s.push_back(make_pair(str[i - 1], size));
                size = 1;
                continue;
            }
            size++;
        }
        s.push_back(make_pair(str.back(), size));
        size = 1;
        cin >> str;
        for (int i = 1; i < n; ++i) {
            if (str[i] != str[i - 1]) {
                r.push_back(make_pair(str[i - 1], size));
                size = 1;
                continue;
            }
            size++;
        }
        r.push_back(make_pair(str.back(), size));
//        cout << "s = \n";
//        for (auto p: s) {
...
//        }
        ...
        for (int i = 0; i < s.size() - 1; ++i) {
            if (s[i].first != r[i].first) {
                result = false;
                break;
            }
            cnt += abs(s[i].second - r[i].second);
            s[i + 1].second += (s[i].second - r[i].second);
        }
        if (s.back().first != r.back().first)result = false;
