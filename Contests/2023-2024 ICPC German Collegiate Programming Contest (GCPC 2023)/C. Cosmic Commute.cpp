...
vll BFSCosmic(int s, vector<vector<int>> &adj) {
    vll len(adj.size(), oo);
    queue<int> q;
    q.push(s);
    len[s] = 0;
...
//    freopen("gcd.in", "r", stdin);
    ...
    vll lenSrc = BFSCosmic(1, adj);
    vll lenDest = BFSCosmic(n, adj);
    ll minRes = oo;
    ll sumDes = 0;
    for (int i = 0; i < k; ++i) {
        sumDes += lenDest[wormhole[i]];
    }
    for (int i = 0; i < k; ++i) {
        minRes = min(minRes, lenSrc[wormhole[i]] * (k - 1) + sumDes - lenDest[wormhole[i]]);
    }

    if (lenSrc[n] * (k - 1) <= minRes) {
        cout << lenSrc[n] << "/1";
    } else {
        ll gcd = __gcd(minRes, k - 1);
        cout << minRes / gcd << "/" << (k - 1) / gcd;
    }
    return 0;
}