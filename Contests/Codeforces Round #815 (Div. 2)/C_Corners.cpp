...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            for (int j = 0; j < m - 1; ++j) {
                int sum = v[i][j + 1] + v[i + 1][j] + v[i + 1][j + 1] + v[i][j] - 4 * '0';
                if (sum <= 2) {
                    diff = max(diff, 2);
                } else if (sum == 3) {
                    diff = max(diff, 1);
...
//        cout << "diff" << diff << "  zeros" << cntZ << endl;
        if (diff == 0)cout << n * m - 2 << endl;
        else if (diff == 2) cout << n * m - (cntZ) << endl;
        else cout << n * m - (cntZ) - 1 << endl;
    }
    return 0;
}
