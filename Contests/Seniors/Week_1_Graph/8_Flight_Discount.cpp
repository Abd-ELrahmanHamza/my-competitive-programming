...
vector<bool> BFSBipart(int s, vector<vector<int>> &graph, vector<bool> &color, vector<bool> &visited) {
    queue<pair<int, int>> q;
    q.push({s, 0});
    int cur, dep;
    while (!q.empty()) {
...
vll dijkstra(vector<vector<edge>> &adjlist, int src) {
    int n = adjlist.size();
...
//    freopen("gcd.in", "r", stdin);
    ...
    for (int i = 0; i < m; ++i) {
        ll x, y, w;
        cin >> x >> y >> w;
        graph[x - 1].push_back(edge(x - 1, y - 1, w));
        reversedGraph[y - 1].push_back(edge(y - 1, x - 1, w));
    }
    auto startDist = dijkstra(graph, 0);
    auto endDist = dijkstra(reversedGraph, n-1);
    ll minDist = LONG_LONG_MAX;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < graph[i].size(); j++) {
            if (startDist[i] == LONG_LONG_MAX || endDist[graph[i][j].to] == LONG_LONG_MAX)continue;
            minDist = min(startDist[i] + graph[i][j].w / 2 + endDist[graph[i][j].to], minDist);
        }
    }
    cout << minDist;
    return 0;
