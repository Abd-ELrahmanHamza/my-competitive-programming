...
void dfs(int i, int j, int n, int m, vector<vector<char>> &graph) {
    graph[i][j] = '#';
    for (int k = 0; k < 4; ++k) {
        if (isValid(i + dirx[k], j + diry[k], n, m) && graph[i + dirx[k]][j + diry[k]] == '.') {
            dfs(i + dirx[k], j + diry[k], n, m, graph);
        }
    }
}

int connectedComponents(int n, int m, vector<vector<char>> &graph) {
    int cnt = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (graph[i][j] == '.') {
...
//    freopen("gcd.in", "r", stdin);
    ...
    cout << connectedComponents(n, m, graph);
    return 0;
}
