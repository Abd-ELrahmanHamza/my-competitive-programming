...
vector<char> BFSPath(pair<int, int> start, pair<int, int> end, vector<vector<char>> &graph) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
//            cout << cur.first + dirx[i] << " " << cur.second + diry[i] << " "
//                 << graph[cur.first + dirx[i]][cur.second + diry[i]] << endl;
...
//                    cout << "here\n";
                    q.push({{cur.first + dirx[i], cur.second + diry[i]}, dep + 1});
...
//    freopen("gcd.in", "r", stdin);
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            cin >> graph[i][j];
            if (graph[i][j] == 'A')start.first = i, start.second = j;
            if (graph[i][j] == 'B')end.first = i, end.second = j;
        }
    }
    auto path = BFSPath(start, end, graph);
    if (path.size() == 1) {
        cout << "NO";
        return 0;
    }
