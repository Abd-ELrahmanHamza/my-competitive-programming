...
void dfs(int node, vector<vector<int>> &graph, vi &start, vi &end) {
    start[node] = timer++;
    route.push_back(node);
    for (int i = 0; i < graph[node].size(); ++i) {
        if (start[graph[node][i]] == -1) {
            dfs(graph[node][i], graph, start, end);
        } else {
            if (end[graph[node][i]] == -1 && res == -1) {
                res = graph[node][i];
                route.push_back(graph[node][i]);
            }
        }
    }
    end[node] = timer++;
...
//    freopen("gcd.in", "r", stdin);
    ...
    for (int i = 0; i < route.size(); ++i) {
        if (route[i] == res && found) {
            result.push_back(route[i] + 1);
            break;
        }
        if (route[i] == res)found = true;
        if (found) {
            result.push_back(route[i] + 1);
        }
    }
