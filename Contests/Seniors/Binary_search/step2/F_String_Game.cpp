...
bool good(string t, string p, int m, vi indices) {
//    cout << "m = " << m << endl;
    int j = 0;
    vi sortedIndices = {indices.begin() + m, indices.end()};
    sort(sortedIndices.begin(), sortedIndices.end());
//    printV(sortedIndices);
//    cout << endl;
    for (int i = 0; i < sortedIndices.size(); ++i) {
//        cout << t[sortedIndices[i] - 1] << " " << p[j] << endl;
        if (t[sortedIndices[i] - 1] == p[j])
            j++;
        if (j == p.size())break;
    }
    return j == p.size();
