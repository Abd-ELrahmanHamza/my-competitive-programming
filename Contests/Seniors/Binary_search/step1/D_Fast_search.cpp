...
int first_less_or_equal(int l, int r, int x, vi &v) {
    while (l + 1 < r) {
        int m = (r + l) / 2;
        if (v[m] <= x) {
            l = m;
...
int first_greater_or_equal(int l, int r, int x, vi &v) {
    while (l + 1 < r) {
        int m = (r + l) / 2;
        if (v[m] < x) {
            l = m;
...
//    freopen("gcd.in", "r", stdin);
    ...
    while (k--) {
        int l, r;
        cin >> l >> r;
        int ll = first_greater_or_equal(-1, n, l, v);
        int rr = first_less_or_equal(-1, n, r, v);
//        cout << ll << " " << rr << endl;
        cout << rr - ll + 1 << " ";
    }
