...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            multiset<int> sa, sb;
            if (a[i] <= lasta)sa.insert(lasta);
            if (a[i] <= lastb)sa.insert(lastb);
            if (b[i] <= lasta)sb.insert(lasta);
            if (b[i] <= lastb)sb.insert(lastb);
            if (sa.size() == 1 && sb.size() == 1) {
                for (int x: sa) {
                    if (sb.count(x) != 0)sb.erase(x);
                }
            }
            if (sa.size() == 0 || sb.size() == 0)res = false;
