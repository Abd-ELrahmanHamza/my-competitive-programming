...
bool isWinning(string s) {
    bool rows = (s[0] == s[1] && s[1] == s[2] && s[0] != '.') || (s[3] == s[4] && s[4] == s[5] && s[3] != '.') ||
                (s[6] == s[7] && s[7] == s[8] && s[6] != '.');

    bool cols = (s[0] == s[3] && s[3] == s[6] && s[0] != '.') || (s[1] == s[4] && s[4] == s[7] && s[1] != '.') ||
                (s[2] == s[5] && s[5] == s[8] && s[2] != '.');
    bool diags = (s[0] == s[4] && s[4] == s[8] && s[0] != '.') || (s[2] == s[4] && s[4] == s[6] && s[2] != '.');
    return rows || cols || diags;
}

bool ticTacToe(string board) {
    ...
    for (; !q.empty(); ++dep, sz = q.size()) {
        while (sz--) {
            ...
            q.pop();
            if (cur == board)return true;
            if (isWinning(cur))continue;
            for (int i = 0; i < 9; ++i) {
                if (cur[i] == '.') {
                    string temp = cur;
                    temp[i] = (turn ? 'X' : 'O');
