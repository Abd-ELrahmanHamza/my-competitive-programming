...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        char winner = -1;
        if (s[0] != '.' && s[0] == s[4] && s[0] == s[8])winner = s[0];
        if (s[2] != '.' && s[2] == s[4] && s[2] == s[6])winner = s[2];

        if (s[0] != '.' && s[0] == s[1] && s[0] == s[2])winner = s[0];
        if (s[3] != '.' && s[3] == s[4] && s[3] == s[5])winner = s[3];
        if (s[6] != '.' && s[6] == s[7] && s[6] == s[8])winner = s[6];

        if (s[0] != '.' && s[0] == s[3] && s[0] == s[6])winner = s[0];
        if (s[1] != '.' && s[1] == s[4] && s[1] == s[7])winner = s[1];
        if (s[2] != '.' && s[2] == s[5] && s[2] == s[8])winner = s[2];
