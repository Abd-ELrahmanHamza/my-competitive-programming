...
int sendingEmailDijkstra(vector<vector<edge>> &adj, int src, int dest) {
    int n = adj.size();
    vi dist(n, oo);
    dist[src] = 0;
    priority_queue<edge> q;
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < adj[e.to].size(); ++i) {
            edge ne = adj[e.to][i];
            if (ne.w + dist[ne.from] < dist[ne.to]) {
...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        int res = sendingEmailDijkstra(adj, start, end);
        if (res == oo)cout << "Case #" << p << ": " << "unreachable\n";
        else cout << "Case #" << p << ": " << res << "\n";
        p++;
    }
    return 0;
}