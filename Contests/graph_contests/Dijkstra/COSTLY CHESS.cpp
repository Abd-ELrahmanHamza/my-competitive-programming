...
bool validBfs(vector<int> &bfs, vector<vector<int>> &graph) {
    ...
    int curOrder = -1;
    for (; !q.empty(); ++dep, sz = q.size()) {
        if (cur >= bfs.size())break;
        set<int> temps;
        while (sz--) {
            if (cur >= bfs.size())break;
            if (q.count(bfs[cur]) == 0) {
                return false;
            }
            curOrder = order[bfs[cur]];
            q.erase(bfs[cur]);
            for (int i = 0; i < graph[bfs[cur]].size(); ++i) {
                if (!visited[graph[bfs[cur]][i]]) {
                    visited[bfs[cur]] = true;
                    temps.insert(graph[bfs[cur]][i]);
                    order[graph[bfs[cur]][i]] = cur + 1;
                }
            }
            cur++;
        }
    ...
    for (int i = 1; i < bfs.size(); ++i) {
        if (order[bfs[i]] < order[bfs[i - 1]])return false;
    }
...
struct edgePoint {
    ...
    edgePoint(pair<int, int> from, pair<int, int> to, ll w) :
            from(std::move(from)), to(std::move(to)), w(w) {}

    bool operator<(edgePoint e) const {
        return w > e.w;
    }
};

int dxemail[] = {-1, -2, -2, -1, 1, 2, 2, 1};
int dyemail[] = {-2, -1, 1, 2, -2, -1, 1, 2};

bool valid(int x, int y, int n, int m) {
    return x >= 0 && y >= 0 && x < n && y < m;
}

int sendingEmailDijkstra(pair<int, int> src, pair<int, int> dest) {
    vector<vector<int>> dist(8, vector<int>(8, oo));
    dist[src.first][src.second] = 0;
    priority_queue<edgePoint> q;
    q.push(edgePoint({-1, -1}, src, 0));
    while (!q.empty()) {
        edgePoint e = q.top();
        q.pop();
        auto curSrc = e.to;
        if (e.w > dist[curSrc.first][curSrc.second])continue;
        for (int i = 0; i < 8; ++i) {
            auto curDest = make_pair(e.to.first + dxemail[i], e.to.second + dyemail[i]);
            if (valid(curDest.first, curDest.second, 8, 8)) {
                int w = curDest.first * curSrc.first + curDest.second * curSrc.second;
                edgePoint ne = edgePoint(curSrc, curDest, w);
                if (w + dist[curSrc.first][curSrc.second] < dist[curDest.first][curDest.second]) {
                    ne.w = dist[curDest.first][curDest.second] = w + dist[curSrc.first][curSrc.second];
                    q.push(ne);
                }
            }
        }
...
//    freopen("gcd.in", "r", stdin);
    ...
    while (cin >> src.first >> src.second >> dest.first >> dest.second) {
        int res = sendingEmailDijkstra(src, dest);
        if (res == oo)
            cout << -1 << endl;
        else
            cout << res << endl;
