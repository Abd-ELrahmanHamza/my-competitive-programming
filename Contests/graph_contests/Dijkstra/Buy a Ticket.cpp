...
struct edge {
    ...
    edge(ull from, ull to, ull w) : from(from), to(to), w(w) {}
...
void highWaysDijkstra(vector<vector<edge>> &adj, vll &dist) {
    int n = adj.size();
    priority_queue<edge> q;
    for (int i = 1; i < n; ++i) {
        q.push(edge(-1, i, dist[i]));
    }
    vector<bool> visited(n);
    while (!q.empty()) {
        edge e = q.top();
        ...
        for (int i = 0; i < adj[e.to].size(); ++i) {
            edge ne = adj[e.to][i];
            if (ne.w + e.w < dist[ne.to]) {
                ne.w = dist[ne.to] = ne.w + e.w;
                q.push(ne);
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < m; ++i) {
        ll u, v, c;
        cin >> u >> v >> c;
        adj[u].push_back(edge(u, v, 2 * c));
        adj[v].push_back(edge(v, u, 2 * c));
    }
    for (int i = 1; i <= n; ++i) {
        cin >> cost[i];
    }
    highWaysDijkstra(adj, cost);
    for (int i = 1; i <= n; ++i) {
        cout << cost[i] << " ";
    }
    return 0;
