...
int mazeDijkstra(vector<vi> &v, int n, int m) {
    vector<vector<int>> dist(n, vector<int>(m, oo));
    dist[0][0] = v[0][0];
    priority_queue<edgePoint> q;
    q.push(edgePoint({-1, -1}, {0, 0}, v[0][0]));
    while (!q.empty()) {
        edgePoint e = q.top();
        q.pop();
        auto curSrc = e.to;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (valid(curDest.first, curDest.second, n, m)) {
                int w = v[curDest.first][curDest.second];
