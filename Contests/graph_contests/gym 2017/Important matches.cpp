...
vector<int> getCount(vector<vector<vector<int>>> &preSumGrid, int a, int b, int c, int d) {
    vector<int> cnt(2001);
    for (int i = 1; i <= 2000; ++i) {
        cnt[i] = preSumGrid[c][d][i] - preSumGrid[c][b - 1][i] - preSumGrid[a - 1][d][i] + preSumGrid[a - 1][b - 1][i];
    }
    return cnt;
}

int getMedian(vector<int> &cnt, int totalCnt) {
    int curCnt = 0;
    int med = totalCnt / 2 + 1;
    for (int i = 1; i <= 2000; ++i) {
        curCnt += cnt[i];
        if (curCnt >= med) {
            return i;
...
int main() {
    ...
    for (int i = 0; i < t; ++i) {
        ...
        vector<vector<vector<int>>> preSumGrid(n + 1, vector<vector<int>>(m + 1, vector<int>(2001)));
        for (int j = 1; j <= n; ++j) {
            for (int k = 1; k <= m; ++k) {
                cin >> grid[j][k];
            }
        ...
        for (int j = 1; j <= n; ++j) {
            for (int k = 1; k <= m; ++k) {
                for (int l = 1; l <= 2000; ++l) {
                    preSumGrid[j][k][l] =
                            preSumGrid[j - 1][k][l] + preSumGrid[j][k - 1][l] - preSumGrid[j - 1][k - 1][l] +
                            (grid[j][k] == l);
                }
            }
        }
        ...
        while (q--) {
            int a, b, c, d;
            cin >> a >> b >> c >> d;
            vi cnt = getCount(preSumGrid, a, b, c, d);
            cout << getMedian(cnt, (d - b + 1) * (c - a + 1)) << "\n";
        }
    }
}