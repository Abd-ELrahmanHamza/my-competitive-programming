...
void mazedfs(vector<vector<char>> &grid, int i, int j, const int &y, int &curCnt) {
    grid[i][j] = 'q';
    for (int k = 0; k < 4; ++k) {
        if (valid(i + mazedx[k], j + mazedy[k], grid.size(), grid[0].size()) &&
            grid[i + mazedx[k]][j + mazedy[k]] == '.') {
            mazedfs(grid, i + mazedx[k], j + mazedy[k], y, curCnt);
        }
    }
    if (curCnt < y) {
        curCnt++;
