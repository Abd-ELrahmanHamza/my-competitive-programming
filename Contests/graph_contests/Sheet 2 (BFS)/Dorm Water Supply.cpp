...
int waterDfs(vector<pair<int, int>> &adj, vector<bool> &vis, int node) {
    vis[node] = true;
    if (adj[node].second == -1) {
        last = node;
        return INT_MAX;
    }
    if (vis[adj[node].first])return -1;
    return min(adj[node].second, waterDfs(adj, vis, adj[node].first));
}
...
//    freopen("gcd.in", "r", stdin);
    ...
    vector<bool> vis(n + 1);
    vector<pair<pair<int, int>, int>> ress;
    for (int i = 1; i <= n; ++i) {
        if (!hasInput[i]) {
            int res = waterDfs(adj, vis, i);
            if (res != -1 && i != last) {
                ress.push_back({{i, last}, res});
            }
        }
    }
    cout << ress.size() << "\n";
    for (int i = 0; i < ress.size(); ++i) {
        cout << ress[i].first.first << " " << ress[i].first.second << " " << ress[i].second << endl;
    }
    return 0;
}