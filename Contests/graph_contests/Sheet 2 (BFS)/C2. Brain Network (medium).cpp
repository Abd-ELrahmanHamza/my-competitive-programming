...
vi brainBFS(vector<vector<int>> &adj, int start) {
    queue<int> q;
...
pair<int, int> treeDiameter(vector<vector<int>> &adj, int i, int par = -1) {
    int diam = 0;
    int maxHeights[3] = {-1, -1, -1};
    for (int j = 0; j < adj[i].size(); ++j) {
        if (adj[i][j] != par) {
            pair<int, int> p = treeDiameter(adj, adj[i][j], i);
            diam = max(diam, p.first);
            maxHeights[0] = p.second + 1;
            sort(maxHeights, maxHeights + 3);
        }
    }
    for (int j = 0; j < 3; ++j) {
        if (maxHeights[j] == -1)maxHeights[j] = 0;
    }
    diam = max(diam, maxHeights[1] + maxHeights[2]);
    return {diam, maxHeights[2]};
}
...
//    freopen("gcd.in", "r", stdin);
    ...
    vi len = brainBFS(adj, 1);

    int maxInd = max_element(len.begin(), len.end()) - len.begin();
    len = brainBFS(adj, maxInd);
