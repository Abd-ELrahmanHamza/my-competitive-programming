...
int dfsgrid(vector<string> &grid, int x, int y) {
    grid[x][y] = 'x';
    int res = 1;
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            if (val(x + i, y + j, grid.size(), grid[0].size()) && grid[x + i][y + j] == '1') {
                res += dfsgrid(grid, x + i, y + j);
            }
        }
    }
    return res;
    ...
    while (t--) {
        ...
        for (int i = 0; i < grid.size(); ++i) {
            for (int j = 0; j < grid[0].size(); ++j) {
                if (grid[i][j] == '1') {
                    maxNum = max(dfsgrid(grid, i, j), maxNum);
                }
            }
        }
        cout << maxNum << "\n";
