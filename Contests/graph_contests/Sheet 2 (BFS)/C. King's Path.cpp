...
int kingBFS(pair<int, int> start, pair<int, int> end, set<pair<int, int>> &graph) {
    queue<pair<int, int>> q;
    q.push(start);
    int dep = 0;
    int sz = 1;
    ...
    for (; !q.empty(); ++dep, sz = q.size()) {
        while (sz--) {
            auto top = q.front();
            if (len[top] != 0 || graph.count(top) == 0) {
                q.pop();
                continue;
            }
            q.pop();
            len[top] = dep;
            for (int i = 0; i < 8; ++i) {
                pair<int, int> newPoint = {top.first + dx[i], top.second + dy[i]};
                q.push(newPoint);
            }
        }
    }
