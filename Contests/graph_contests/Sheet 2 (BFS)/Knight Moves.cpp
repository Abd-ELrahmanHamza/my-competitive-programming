...
int chessBFS(string p1, string p2) {
...
//        cout << top.first << " " << top.second << endl;
        if (chess[top.first[0] - 'a'][top.first[1] - '1'])continue;
        chess[top.first[0] - 'a'][top.first[1] - '1'] = true;
        if (top.first[0] == p2[0] && top.first[1] == p2[1]) {
            return top.second;
        }
        if (top.first[0] - 1 >= 'a' && top.first[1] - 2 >= '1')
            q.push({string({(char) (top.first[0] - 1), (char) (top.first[1] - 2)}), top.second + 1});
        if (top.first[0] - 1 >= 'a' && top.first[1] + 2 <= '8')
            q.push({string({(char) (top.first[0] - 1), (char) (top.first[1] + 2)}), top.second + 1});
        if (top.first[0] - 2 >= 'a' && top.first[1] - 1 >= '1')
            q.push({string({(char) (top.first[0] - 2), (char) (top.first[1] - 1)}), top.second + 1});
        if (top.first[0] - 2 >= 'a' && top.first[1] + 1 <= '8')
            q.push({string({(char) (top.first[0] - 2), (char) (top.first[1] + 1)}), top.second + 1});

        if (top.first[0] + 1 <= 'h' && top.first[1] - 2 >= '1')
            q.push({string({(char) (top.first[0] + 1), (char) (top.first[1] - 2)}), top.second + 1});
        if (top.first[0] + 1 <= 'h' && top.first[1] + 2 <= '8')
            q.push({string({(char) (top.first[0] + 1), (char) (top.first[1] + 2)}), top.second + 1});
        if (top.first[0] + 2 <= 'h' && top.first[1] - 1 >= '1')
            q.push({string({(char) (top.first[0] + 2), (char) (top.first[1] - 1)}), top.second + 1});
        if (top.first[0] + 2 <= 'h' && top.first[1] + 1 <= '8')
            q.push({string({(char) (top.first[0] + 2), (char) (top.first[1] + 1)}), top.second + 1});
...
//    freopen("gcd.in", "r", stdin);
    ...
    while (cin >> p1 >> p2) {
        cout << "To get from " << p1 << " to " << p2 << " takes " << chessBFS(p1, p2) << " knight moves." << endl;
    }
    return 0;
}