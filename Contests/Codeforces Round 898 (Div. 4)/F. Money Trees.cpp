...
bool pantsDfs(string country, string country2, unordered_map<string, vector<string>> &adj,
    ...
    for (auto c: adj[country]) {
        if (!visited[c]) {
            if (pantsDfs(c, country2, adj, visited))return true;
        }
    }
    return false;
}

ll computeDiff(vi &v, ll h) {
    ll sum = 0;
    for (int i = 0; i < v.size(); ++i) {
        sum += max(0LL, h - v[i]);
    }
...
//        cout << endl;
        for (int i = 0; i < n; ++i) {
            ...
            while (l < r - 1) {
                int mid = (l + r) / 2;
//                cout << "l = " << l << " sum = " << preSum[mid + 1] - preSum[i] << " mid = " << mid << " \n";
                if (preSum[mid + 1] - preSum[i] <= k) {
                    ans = mid - i + 1;
                    l = mid;
                } else {
                    r = mid;
