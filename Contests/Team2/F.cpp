...
void next(int &lastRow, int &lastCol, int n, int hashes) {
    lastCol = ceil(hashes / double(n));
    lastRow = hashes % n;
    lastRow = (lastRow == 0 ? n : lastRow);
}
...
int main() {
    ...
    int lastCol = ceil(hashes / double(n)), lastRow = hashes % n;
    lastRow = (lastRow == 0 ? n : lastRow);

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (v[i][j] == '.' && (j < lastCol - 1 || (j == lastCol - 1 && i <= lastRow - 1))) {
                faults++;
            }
        }
    }
    while (q--) {
        ...
        if (v[r - 1][c - 1] == '.') {
            hashes++;
            next(lastRow, lastCol, n, hashes);
            if (v[lastRow - 1][lastCol - 1] == '.') {
                faults++;
            }
            if ((c - 1) * n + r <= hashes)faults--;
            v[r - 1][c - 1] = '*';
        } else {
            if (v[lastRow - 1][lastCol - 1] == '.') {
                faults--;
