...
//    freopen("commandos.in", "r", stdin);
    ...
    while (t--) {
        int Q, M;
        cin >> Q >> M;
        vector<vector<bool>> v(Q, vector<bool>(4,true));
        ...
        for (int i = 0; i < Q; ++i) {
            ...
            if (int(v[i][0]) + int(v[i][1]) + int(v[i][2]) + int(v[i][3]) == 1) {
                results[i] = v[i][0] ? 'A' : results[i];
                results[i] = v[i][1] ? 'B' : results[i];
                results[i] = v[i][2] ? 'C' : results[i];
                results[i] = v[i][3] ? 'D' : results[i];
            }
        }
        for (int i = 0; i < Q; ++i) {
            if (results[i] != 0)cout << results[i];
            else cout << '?';
            if(i!=Q-1)cout<<" ";
        }
        cout << endl;
