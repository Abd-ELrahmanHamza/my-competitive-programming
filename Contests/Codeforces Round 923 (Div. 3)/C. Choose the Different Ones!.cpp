...
int main() {
    ...
    while (t--) {
        ...
        auto alower = std::upper_bound(a.begin(), a.end(), k);
        auto blower = std::upper_bound(b.begin(), b.end(), k);
        int lena = alower - a.begin(), lenb = blower - b.begin();
        if (lena < k / 2 || lenb < k / 2) {
            cout << "NO\n";
            continue;
        }
        vi c(k * 2);
