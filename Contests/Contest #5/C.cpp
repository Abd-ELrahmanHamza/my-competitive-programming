...
bool getValue(const set<int> &s, vi nums) {
    for (auto x: s) {
...
//    3:1
    ...
    for (ll mask = 0; mask < (1 << q); mask++) {
        set<int> s;
        // mask = 110
        for (ll i = 0; i < q; ++i) { //mask :    0         0       1
            // 001 & 110 = 0         //      lucky[2]  lucky[1]  lucky[0]
            // 010 & 110 = 1
            // 100 & 110 = 1
            if ((1 << i) & mask) {
                s.insert(lucky[i]);
...
*/
    ...
    set<int> resS;
    for (set<int> temps: vs) {
        bool tempRes = getValue(temps, nums);
        if (tempRes && temps.size() > result) {
            result = temps.size();
            resS = temps;
        }
    }
