...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        vi v(n);
        readV(v);
        int onePos = min_element(v.begin(), v.end()) - v.begin();
        int greatestPos = max_element(v.begin(), v.end()) - v.begin();
        if (onePos == 0) {
            cout << 2 << " " << greatestPos + 1 << endl;
        } else if (onePos == n - 1) {
            cout << n - 1 << " " << greatestPos + 1 << endl;
        } else {
            int twoPos = find(v.begin(), v.end(), 2) - v.begin();
            if (greatestPos != n - 1 && greatestPos != 0) {
                if (greatestPos > onePos) {
                    cout << twoPos + 1 << " " << greatestPos + 2 << endl;
                } else {
                    cout << twoPos + 1 << " " << greatestPos << endl;
                }
            } else {
                if ((greatestPos > onePos && twoPos > onePos) || (greatestPos < onePos && twoPos < onePos)) {
                    cout << greatestPos + 1 << " " << twoPos + 1 << endl;
                } else {
                    cout << greatestPos + 1 << " " << onePos + 1 << endl;
                }
            }
        }
    }
