...
int initCost(vector<vi> &adj, vector<bool> &visited, int node, vll &cost) {
    visited[node] = true;
    ll c = 0;
    bool isLeaf = true;
    for (int i = 0; i < adj[node].size(); ++i) {
        if (!visited[adj[node][i]]) {
            isLeaf = false;
            c += initCost(adj, visited, adj[node][i], cost);
        }
    }
    return cost[node] = c + (isLeaf ? 1 : 0);
}
...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        ...
        initCost(adj, visited, 0, numLeafs);
//        printV(numLeafs);
//        cout << endl;
        int q;
        cin >> q;
        while (q--) {
            int u, v;
            cin >> u >> v;
            cout << numLeafs[u - 1] * numLeafs[v - 1] << endl;
        }
    }
