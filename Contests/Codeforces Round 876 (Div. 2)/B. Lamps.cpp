...
//    freopen("gcd.in", "r", stdin);
    ...
    while (t--) {
        int n;
        cin >> n;
        priority_queue<pair<int, int>, std::vector<std::pair<int, int>>, Compare> allLamps, lit;  // Using custom sorting function
        for (int i = 0; i < n; ++i) {
            int a, b;
            cin >> a >> b;
            allLamps.push({a, b});
        ...
        int x = 0;
        ll score = 0;
        while (!allLamps.empty()) {
//            cout << allLamps.top().first << " " << allLamps.top().second << endl;
            score += allLamps.top().second;
            lit.push(allLamps.top());
            allLamps.pop();
            x++;
            int cnt = 0;
            while (!allLamps.empty() && allLamps.top().first <= x) {
                allLamps.pop();
            }
            while (!lit.empty() && lit.top().first <= x) {
                lit.pop();
                cnt++;
            }
            x -= cnt;
