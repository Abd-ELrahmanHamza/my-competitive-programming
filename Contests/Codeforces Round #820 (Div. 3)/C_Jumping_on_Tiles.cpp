...
bool helpComp(const pair<char, int> &p1, const pair<char, int> &p2) {
    if (p1.first < p2.first)return true;
    else if (p1.first > p2.first)return false;
    return p1.second < p2.second;
}
...
int main() {
    ...
    while (t--) {
        ...
        char start = s.front(), end = s.back();
        for (int i = 0; i < s.size(); ++i) {
            if (start < end && s[i] >= start && s[i] <= end)
                ss.insert({s[i], i + 1});
            else if (start >= end && s[i] <= start && s[i] >= end)ss.insert({s[i], i + 1});
        }

        cout << abs(end - start) << " " << ss.size() << endl;
        if (start <= end) {
            cout << 1 << " ";
            for (auto it = next(ss.begin()); it != prev(ss.end()); it++) {
                if (it->second != 1 && it->second != s.size()) {
                    cout << it->second << " ";
                } else if (it->second == 1) {
                    cout << ss.begin()->second << " ";
                } else if (it->second == s.size()) {
                    cout << prev(ss.end())->second << " ";
                }
            }
            cout << s.size();
        } else {
            cout << 1 << " ";
            for (auto it = next(ss.rbegin()); it != prev(ss.rend()); it++) {
                if (it->second != 1 && it->second != s.size()) {
                    cout << it->second << " ";
                } else if (it->second == s.size()) {
                    cout << ss.begin()->second << " ";
