...
bool ecDist(int x1, int y1, int x2, int y2, int r) {
...
//    freopen("gcd.in", "r", stdin);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = minX; j <= maxX; ++j) {
            if (ecDist(j, minY, x, y, r))res.insert({j, minY});
        }

        for (int j = minX; j <= maxX; ++j) {
            if (ecDist(j, maxY, x, y, r))res.insert({j, maxY});
        }

        for (int j = minY; j <= maxY; ++j) {
            if (ecDist(minX, j, x, y, r))res.insert({minX, j});
        }

        for (int j = minY; j <= maxY; ++j) {
            if (ecDist(maxX, j, x, y, r))res.insert({maxX, j});
        }
    }
    cout << (maxY - minY + 1) * 2 + (maxX - minX - 1) * 2 - res.size();
    return 0;
}
