...
void UVAMarcos(int i, int j, int index) {
    if (graph[i][j] == '.')return;
    if (found)return;
    if (index == 7) {
        found = true;
    ...
    graph[i][j] = '.';
    if (isValid(i - 1, j) && graph[i - 1][j] == s[index]) {
        cout << "forth";
        if (index != 6)
            cout << " ";
        UVAMarcos(i - 1, j, index + 1);
    }
    if (isValid(i, j + 1) && graph[i][j + 1] == s[index]) {
        cout << "right";
        if (index != 6)
            cout << " ";
        UVAMarcos(i, j + 1, index + 1);
    }
    if (isValid(i, j - 1) && graph[i][j - 1] == s[index]) {
        cout << "left";
        if (index != 6)
